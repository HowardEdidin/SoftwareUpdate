...
namespace SoftwareUpdate
{
	public static class Functions
	{
		...
		public static async Task<string> UpdateSecureTask(
		    string input,
			...
			ILogger logger)
		{
			...
			var instanceId = await client.StartNewAsync(nameof(UpdateSecure), input: input);
			...
			[OrchestrationTrigger] DurableOrchestrationContext context)
		{
			...
			var doc = await context.CallActivityAsync<dynamic>(nameof(GetDoc), deviceId);
			...
			var cal = new DeviceMetric.CalibrationComponent
			{
				...
				State = null,      // values: not-calibrated,  calibration-required, calibrated, unspecified see https://www.hl7.org/fhir/valueset-metric-calibration-state.html
