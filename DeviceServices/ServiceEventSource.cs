...
{
	[EventSource(Name = "MyCompany-SoftwareUpdateService-DeviceServices")]
	internal sealed class ServiceEventSource : EventSource
	{
		...
		public static class Keywords
		{
			...
			public const EventKeywords SERVICE_INITIALIZATION = (EventKeywords) 0x2L;
		}
		...
		{
			if (IsEnabled()) WriteEvent(MessageEventId, message);
		}
 ...
 SizeInBytes(applicationTypeName) };
		...
		[Event(ServiceTypeRegisteredEventId, Level = EventLevel.Informational,
			Message = "Service host process {0} registered service type {1}", Keywords = Keywords.SERVICE_INITIALIZATION)]
		...
		[Event(ServiceHostInitializationFailedEventId, Level = EventLevel.Error,
			Message = "Service host initialization failed", Keywords = Keywords.SERVICE_INITIALIZATION)]
